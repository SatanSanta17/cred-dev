...
- **Follow the development process — no shortcuts.** Every change follows this sequence: (1) Understand the emotion — why does this matter to the user? (2) Write the PRD in `docs/<feature-folder>/prd.md` (3) Get explicit approval on the PRD (4) Write the TRD in `docs/<feature-folder>/trd.md` based on the approved PRD (5) Get explicit approval on the TRD (6) Implement one part at a time in small, pushable increments (7) Review, merge, move to the next part. No step is skipped. **PRD and TRD are sequential, not simultaneous** — the TRD is never written before the PRD is approved, because PRD changes would cascade into TRD rework.
- **No code changes without a documented PRD and TRD.** Every feature, fix, or refactor must have a written PRD (Product Requirements Document) approved first, then a TRD (Technical Requirements Document) approved second, before any code is touched. No exceptions — not even "small" changes. **One exception:** trivial fixes (typos, CSS tweaks, one-line bugs) may skip the PRD/TRD — but still require explicit approval before any code is changed. **Second exception:** when the PRD is straightforward enough that the "how" is self-evident from the "what" (e.g., file moves, import updates, no new schemas or APIs), the TRD may be skipped with explicit owner approval. Never change code silently.
